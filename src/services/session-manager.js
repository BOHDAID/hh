...
class OSNSessionManager {
  ...
  /**
   * جلب QR Code - يفتح متصفح مؤقت، يحقن الكوكيز، يأخذ QR، ويُغلق
   */
  async getQRCode() {
    ...
    return await this._withBrowser(async (browser) => {
      console.log('📱 Opening browser for QR capture...');
      ...
      try {
        await page.goto('https://osnplus.com/settings/devices', {
          waitUntil: 'domcontentloaded',
          timeout: 20000,
        });
      } catch {
        console.log('⚠️ Devices page slow, continuing...');
      }

      await new Promise(resolve => setTimeout(resolve, 3000));

      const currentUrl = page.url();
      console.log('🔗 Devices page URL:', currentUrl);

      // التحقق إن الجلسة لا تزال صالحة
      ...
      const addButton = await this._findButton(page, ['add', 'إضافة', 'add device', 'إضافة جهاز']);
      if (addButton) {
      ...
      // البحث عن QR Code
      const qrSelectors = [
        'img[alt*="QR" i]', 'img[src*="qr" i]', 'canvas',
        '[data-testid*="qr" i]', '.qr-code', '#qr-code', 'svg[class*="qr" i]',
      ];
